...
class TestPCIC(TestCase):
    ...
    def test_PCIC_client_with_wrong_IP(self):
        with self.assertRaises(TimeoutError):
            s = deviceAddress.split(".")
            newDeviceAddress = ".".join([s[i] for i in range(3)]) + ".1"
            O2x5xxPCICDevice(newDeviceAddress, pcicTcpPort)

    def test_PCIC_client_with_wrong_port(self):
        with self.assertRaises(ConnectionRefusedError):
            O2x5xxPCICDevice(deviceAddress, pcicTcpPort+1)
