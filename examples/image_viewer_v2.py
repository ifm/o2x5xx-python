...
class GrabO2x5xx(object):
    ...
    def read_next_frames(self):
        ...
        self.frames = self.deserialize_image_chunk(self.frames)
...
if __name__ == '__main__':
    ...
    # for k in range(len(grabber.frame_ids)):
    #     ax = fig.add_subplot(rows, cols, k+1)

    figs = [plt.figure() for k in range(len(grabber.frame_ids))]
    print('x')

    # frame_bytes = grabber.frames[0][1]
    # frame_bytes = io.BytesIO(frame_bytes)
    # image = mpimg.imread(frame_bytes, format='jpg')
    # im = plt.imshow(image, animated=True)


    def updatefig(*args):
        """
        Updating the subplots of the figure.
        Source:
        https://stackoverflow.com/questions/42816951/how-to-generate-an-animation-with-images-in-subplots-matplotlib

        :return:
        """
        grabber.read_next_frames()

        # image = mpimg.imread(io.BytesIO(grabber.frames[0][1]), format='jpg')

        # for k in range(len(grabber.frame_ids)-1):
        #     _image = mpimg.imread(io.BytesIO(grabber.frames[k+1][1]), format='jpg')
        #     image = np.concatenate((image, _image), axis=0)
    ...
    for fig in figs:
    # for x, frame_id in enumerate(grabber.frame_ids):
        # ani = animation.FuncAnimation(fig, updatefig, interval=50, blit=True)
        ani = animation.FuncAnimation(fig, updatefig, interval=50, blit=True, save_count=1)
